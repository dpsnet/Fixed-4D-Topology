...
We implement \ref{alg:greedy} with the following specifications:
...
\ref{tab:convergence} validates the $O(\log(1/\eps))$ convergence rate predicted by \ref{thm:convergence_rate}. We approximate $\alpha = \pi - 3$ (an irrational number) to various precisions.
...
\ref{tab:examples} shows approximations of various mathematical constants.
...
To numerically verify \ref{thm:linear_independence}, we check that small rational combinations do not produce exact zeros:
