...
\textbf{Critical Clarification on Probe Energy:}
\begin{itemize}
...
\item The system has a fixed $n_{\text{dof}}$ determined by its intrinsic $E_c$ and system parameters ($\Omega$, $E_b$, $M$, etc.)
\item Only when $E_{\text{probe}} \sim E_c$ and the probe \textbf{perturbs} the system does the effective dimension change---because the system has been \textbf{altered}, not just observed
\item Spectral dimension $d_s(\tau)$ is a mathematical tool for characterizing the system at scale $\tau$; it does not imply the system "flows" when probed differently
\end{itemize}
...
\textbf{Refinement 2: Time-Dependent $E_c(t)$}. For a single system whose parameters evolve in time (e.g., rotation speed $\Omega(t)$, black hole mass $M(t)$, temperature $T(t)$), the constraint energy becomes time-dependent: $E_c = E_c(t)$. The dimension flow formula then describes the \textbf{real dynamical evolution} of the system's effective dimension:
\begin{equation}
