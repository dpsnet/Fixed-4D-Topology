...
\subsection{Constraint-Determined Effective Degrees of Freedom}
\label{subsec:phenomenon}

Physical systems exhibit a fundamental property: their \textbf{effective degrees of freedom are determined by their internal constraints}, not by the energy scale of external probes. Systems with stronger internal constraints exhibit fewer accessible dynamical modes (lower effective dimensionality), while systems with weaker constraints exhibit more accessible modes (higher effective dimensionality). This relationship is intrinsic to the system itself, independent of how it is observed. This constraint-determined behavior manifests across diverse physical contexts, from rapidly rotating fluids to black hole horizons to quantum spacetime geometries. Rather than indicating any change in the topological structure of space, this phenomenon reflects how the \textbf{strength of internal constraints} determines the effective number of accessible dynamical modes.
...
\textbf{Terminological Note}: We intentionally avoid the phrase ``spectral flow'' or ``dimension flow'' in this revised version. In mathematics, ``spectral flow'' (Atiyah-Patodi-Singer) refers specifically to the net number of eigenvalues crossing zero in a family of self-adjoint operators. In physics literature, ``dimension flow'' misleadingly suggests that space itself changes dimension when probed differently. We use \textbf{``mode constraint''} or \textbf{``effective degrees of freedom''} to describe the physical phenomenon: the number of accessible dynamical modes is determined by internal constraints, not by external observation. The spectral dimension $d_s(\tau)$ is a mathematical probe that characterizes this constraint structure.
...
The physical interpretation of varying effective degrees of freedom is \textbf{hierarchical mode constraint}. Consider a system with topological dimension $d_{\text{topo}}$ and global constraint energy $E_c$. The system possesses a hierarchy of internal modes, each with characteristic energy gaps relative to $E_c$:
...
\textbf{Core Concept: Effective Degrees of Freedom are Determined by Constraint Strength}

\begin{center}
\fbox{\parbox{0.95\textwidth}{
\small
\textbf{Fundamental Principle:} The effective degrees of freedom of a physical system are an \textbf{intrinsic property} determined by its internal constraint strength, not by external probe energy.

\textbf{Critical Clarification on Terminology:}
\begin{itemize}
\item \textbf{``Effective dimension''} refers to the number of accessible dynamical modes, not to any change in the topological dimension of space
\item \textbf{``Dimension flow'' is a misleading term} that suggests space itself changes dimension when probed differently; we avoid this terminology
\item \textbf{Probe energy $E_{\text{probe}}$ does NOT change the system's degrees of freedom}---it merely determines which modes the observer can resolve
\item The system has fixed $n_{\text{dof}}$ determined by intrinsic $E_c$; only perturbation changes $n_{\text{dof}}$
\item Spectral dimension $d_s(\tau)$ is a mathematical characterization, not a physical ``flow''
\end{itemize}
...
\textbf{2. Effective Degrees of Freedom} ($n_{\text{dof}}$): The number of accessible dynamical modes, determined by $E_c$:
\begin{itemize}
\item Higher $E_c$ (stronger constraints) $\rightarrow$ Fewer $n_{\text{dof}}$ (more ``frozen'' modes)
\item Lower $E_c$ (weaker constraints) $\rightarrow$ More $n_{\text{dof}}$ (more ``free'' modes)
\end{itemize}
...
\textbf{The parameter $c_1$} describes the \textbf{universal relationship} between constraint strength and effective degrees of freedom: $n_{\text{dof}} = n_{\text{dof}}(E_c; c_1)$.

\textbf{Refinement 1: Scale-Dependent $E_c$}. In complex systems with internal structure, the effective constraint energy may itself depend on scale: $E_c = E_c(\tau)$. This reflects the renormalization group idea that coarse-graining changes the effective parameters.

\textbf{Refinement 2: Time-Dependent $E_c(t)$}. For a single system whose parameters evolve in time (e.g., rotation speed $\Omega(t)$, black hole mass $M(t)$, temperature $T(t)$), the constraint energy becomes time-dependent: $E_c = E_c(t)$. The constraint formula then describes the \textbf{real dynamical evolution} of the system's effective degrees of freedom:
\begin{equation}
n_{\text{dof}}(t) = d_{\text{topo}} - (d_{\text{topo}} - d_{\text{low}}) \cdot \frac{1}{1 + \left(\frac{E_c(t)}{E_{\text{ref}}}\right)^{c_1}}
\end{equation}

\textbf{Three Interpretations of $c_1$:}
\begin{enumerate}
\item \textbf{Cross-system}: Different systems with different parameters ($\Omega_1 \neq \Omega_2$) have different $n_{\text{dof}}$
\item \textbf{Structural hierarchy}: Same system, different internal scales ($E_c(\tau)$ varies with coarse-graining)
\item \textbf{Temporal evolution}: Same system, time-varying parameters ($E_c(t)$ evolves dynamically) $\rightarrow$ $n_{\text{dof}}(t)$
\end{enumerate}

\textit{Note: In all cases, $n_{\text{dof}}$ is an objective property determined by the system's intrinsic state ($E_c$ at that time/scale), not by external probe energy.}
}}
\end{center}
...
A subtle but important aspect of the mode constraint framework is the \textbf{hierarchical structure} of energy-constraint relationships. This involves three distinct levels:
...
This hierarchical structure exhibits a form of \textbf{self-similarity}: the relationship between constraint energy and effective degrees of freedom may reappear at each level, though with different characteristic scales. The parameter $c_1$ describes the universal pattern of how constraint strength determines the number of accessible degrees of freedom across these levels, while the specific constraint energies at each level depend on the particular physics at that scale.

\textbf{Example}: In Cu$_2$O excitons, the quantum defect analysis operates at the Mode Level (exciton states within the crystal lattice). The internal electron-hole structure involves different physics and energy scales. The mode constraint formula captures the universal pattern while respecting these distinct physical origins.
...
The modern application of spectral dimension to quantum gravity began with observations in Causal Dynamical Triangulations (CDT) \cite{Ambjorn2005}, where numerical simulations revealed that different configurations in the quantum spacetime ensemble exhibit different numbers of accessible degrees of freedom at different characteristic scales. This reflects varying constraint strengths across the ensemble---not a ``flow'' of dimension within a single system.

\textbf{Critical Note}: The interpretation of this behavior as ``spacetime becoming two-dimensional'' is a misnomer. The correct interpretation is that different elements of the quantum spacetime ensemble exhibit different effective degrees of freedom due to their intrinsic constraint structures. The spectral dimension characterizes this constraint-determined mode structure---not a change in the topological dimension of any manifold.
