...
\usepackage[left=1cm,right=4cm,top=2.5cm,bottom=2.5cm]{geometry}
\usepackage{fancyhdr,setspace,titlesec,caption}

% Page setup
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small 谱维度流研究}
\fancyhead[R]{\small 王斌与Kimi}
\fancyfoot[C]{\thepage}
\setstretch{1.15}
...
% Title
\title{\vspace{-1cm}\textbf{谱维度流在引力系统中的研究：}\\[0.5em]\Large 量子引力现象学的统一框架}
...
广义相对论与量子力学的调和仍然是理论物理学中最深刻的挑战之一。这一挑战的核心是普朗克尺度下的时空结构问题：如果量子效应在量级为 $\ell_P = \sqrt{G\hbar/c^3} \approx 1.6 \times 10^{-35}$ m, what replaces the smooth manifold description of classical gravity?

\subsection{量子时空问题}

众多理论方法 已经解决了这个问题, 每种方法都提供了独特的见解:

\begin{itemize}
\item \textbf{String Theory:} 在微扰表述中，基本弦在可与弦长相比的尺度上探测时空 $\ell_s = \sqrt{\alpha'}$. The requirement of a consistent critical dimension $D = 10$ or $D = 11$ in M-theory suggests that our four-dimensional world emerges through compactification of extra dimensions. The spectral dimension in string theory exhibits interesting behavior related to the worldsheet formulation.

\item \textbf{Loop Quantum Gravity:} 这种非微扰方法直接量子化几何，导致面积和体积算符的离散谱。 The resulting quantum geometry at the Planck scale resembles a polymer-like structure with $d_s = 2$ generically, though the details depend on the specific semiclassical state chosen.

\item \textbf{Asymptotic Safety:} 渐近安全情景下引力的重整化群流预测了一个非高斯固定点 where the effective dimensionality reduces. Numerical studies in the functional renormalization group framework have consistently found $d_s \approx 2$ at the fixed point.

\item \textbf{Causal Dynamical Triangulations:} 这种方法中随机几何的蒙特卡罗模拟提供了维度降低的确凿证据, with $d_s = 2$ at short distances and a smooth crossover to $d_s = 4$ at large scales.
\end{itemize}
...
本研究是作为Dimensionics人机协作计划的一部分进行的。 We thank the SnapPy development team for making the hyperbolic manifold census available, and the LIGO/Virgo collaboration for public gravitational wave data. Computations were performed using standard numerical libraries (mpmath, numpy, scipy).
